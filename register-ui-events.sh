...
runtime_file="/var/run/cassetteos/message-bus.url"
ui_message_bus_file="/var/lib/cassetteos/ui-message-bus.json"
if [ -f "$runtime_file" ] && [ -f "$ui_message_bus_file" ]
then
    MESSAGE_BUS_URL=$(cat /var/run/cassetteos/message-bus.url)
